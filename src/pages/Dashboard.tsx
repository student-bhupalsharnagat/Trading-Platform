        ...
        {activeNav === 'positions' && (
          <div className="space-y-4 animate-fadeIn">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-400 font-mono">
                Today P&L: <span className="text-emerald-400 font-bold">+₹{portfolio?.wallet?.todayPnL?.toLocaleString('en-IN') || '504.52'}</span>
              </span>
            </div>
            ...
            <div className="space-y-2.5">
              {portfolio?.positions?.map((pos) => (
                <div
                  key={pos.id}
                  className="p-4 bg-[#0B111C] border border-[#1A2638] rounded-xl flex items-center justify-between flex-wrap gap-3"
                >
                  <div>
                    <div className="flex items-center gap-2">
                      <span
                      ...
                      <h3 className="font-extrabold text-sm text-white">{pos.symbol}</h3>
                      <span className="text-xs text-amber-400 font-mono font-bold">
                        {pos.lots || 1} Lot ({pos.qty} Qty)
                      </span>
                    </div>
                    <p className="text-xs text-slate-400 mt-1 font-mono">
                      Avg: ₹{pos.avgPrice?.toLocaleString('en-IN', { minimumFractionDigits: 2 })} • LTP: ₹
                      {pos.ltp?.toLocaleString('en-IN', { minimumFractionDigits: 2 })} • Product: {pos.product}
                    </p>
                  </div>

                  <div className="flex items-center gap-3">
                    <div className="text-right font-mono font-bold">
                      <div className={pos.pnl >= 0 ? 'text-emerald-400 text-sm sm:text-base' : 'text-rose-400 text-sm sm:text-base'}>
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 text-rose-300 text-xs font-bold transition-colors cursor-pointer disabled:opacity-50"
                    >
                      {closingPositionId === pos.id ? 'Closing...' : 'Exit'}
                    </button>
                  </div>
                </div>
              ))}
              ...
              {(!portfolio?.positions || portfolio.positions.length === 0) && (
                <div className="text-center py-12 text-slate-500 text-xs">
                  No open positions right now. Click any instrument to start trading.
                </div>
              )}
            </div>
          </div>
