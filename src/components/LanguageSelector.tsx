    ...
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      <button
        ...
        className={`px-2.5 sm:px-3 py-1.5 rounded-xl border text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer shadow-sm active:scale-95 ${
          isOpen
            ? 'bg-[#152238] border-amber-500/60 text-amber-300 ring-1 ring-amber-500/40'
            : 'bg-[#0E1626] hover:bg-[#152238] border-slate-700/60 text-slate-200 hover:text-white'
        }`}
        title="Select Language"
      >
        <Globe className="w-3.5 h-3.5 text-amber-400 stroke-[2.2]" />
      ...
      {isOpen && (
        <div
          ...
          className="absolute right-0 top-full mt-2 w-56 sm:w-60 bg-[#0F172A] border border-[#1E2E44] rounded-2xl shadow-2xl z-50 overflow-hidden backdrop-blur-xl animate-scaleUp py-1.5 divide-y divide-[#162234]"
          role="menu"
                ...
                <button
                  ...
                  className={`w-full px-4 py-2.5 flex items-center justify-between text-left transition-colors cursor-pointer group ${
                    isSelected
                      ...
                      : 'text-slate-200 hover:bg-[#162234] hover:text-white'
                  }`}
                >
                  ...
                  <span
                    className={`text-xs font-mono transition-colors ${
                      isSelected ? 'text-amber-300/80 font-bold' : 'text-slate-400 group-hover:text-slate-300'
                    }`}
                  >
                    {lang.englishName}
                  </span>
