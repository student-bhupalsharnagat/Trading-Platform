...
export const MarketClosedModal: React.FC<MarketClosedModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-70 bg-black/85 backdrop-blur-sm flex items-center justify-center p-4 animate-fadeIn">
      <div className="bg-[#0D1524] border border-[#1E2B40] rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden p-6 sm:p-7 flex flex-col items-center text-center animate-scaleUp">
        {/* Glowing Clock Icon Badge matching Screenshot */}
        <div className="w-14 h-14 rounded-2xl bg-[#34141B] border border-rose-500/40 flex items-center justify-center shadow-lg shadow-rose-950/50">
        ...
        {/* Description */}
        <p className="text-xs sm:text-sm text-slate-400 font-normal leading-relaxed mt-2.5 px-1">
        ...
        <div className="w-full bg-[#080E18] border border-[#162234] rounded-xl p-3 sm:p-3.5 text-center mt-5">
          <div className="text-xs font-semibold text-slate-400 font-mono">
            {exchangeName} trading hours:{' '}
            <span className="text-amber-400 font-bold">{tradingHours}</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 w-full mt-6">
          <button
            type="button"
            onClick={onClose}
            className="py-3 px-4 rounded-xl bg-[#142032] hover:bg-[#1B2B44] border border-[#203450] text-slate-300 hover:text-white font-bold text-sm transition-colors cursor-pointer"
          >
            Close
          </button>
          <button
